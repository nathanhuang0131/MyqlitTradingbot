...
def load_account_view(root: Path) -> dict[str, Any]:
    ...
    decision_trace = read_csv_safe(root / "decision_trace.jsonl")
    if decision_trace.empty and (root / "decision_trace.jsonl").exists():
        try:
            decision_trace = pd.read_json(root / "decision_trace.jsonl", lines=True)
        except Exception:
            decision_trace = pd.DataFrame()
    position_health = read_csv_safe(root / "position_health.csv")
    ...
    if not position_health.empty and "latest_status" in position_health.columns:
        risk_alerts = position_health[position_health["latest_status"].astype(str).isin(["risk", "invalidated", "watch"])].copy()
    decision_blocks = 0
    if not decision_trace.empty and "blocked" in decision_trace.columns:
        decision_blocks = int(pd.to_numeric(decision_trace["blocked"], errors="coerce").fillna(0).astype(bool).sum())
...
def load_market_view(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "model_attribution": alpha.groupby("symbol", dropna=False).head(1) if not alpha.empty else pd.DataFrame(),
        "signal_funnel": _build_signal_funnel(root),
    }
...
def _build_signal_funnel(root: Path) -> pd.DataFrame:
    stage = read_csv_safe(root / "stage_trace.csv")
    if stage.empty or "stage" not in stage.columns:
        return pd.DataFrame(columns=["stage", "events"])
    return stage.groupby("stage", dropna=False).size().reset_index(name="events")


def load_position_trace_view(root: Path) -> dict[str, Any]:
    journal = read_csv_safe(root / "position_journal.csv")
    health = read_csv_safe(root / "position_health.csv")
    trace = read_csv_safe(root / "decision_trace.jsonl")
    if trace.empty and (root / "decision_trace.jsonl").exists():
        try:
            trace = pd.read_json(root / "decision_trace.jsonl", lines=True)
        except Exception:
            trace = pd.DataFrame()
    merged = journal.copy()
    if not merged.empty and not health.empty and "symbol" in merged.columns and "symbol" in health.columns:
        merged = merged.merge(
            health[
                [c for c in ["symbol", "latest_status", "latest_recommended_action", "reason_summary", "latest_review_timestamp"] if c in health.columns]
            ],
            on="symbol",
            how="left",
            suffixes=("", "_health"),
        )
    return {"journal": journal, "health": health, "trace": trace, "merged": merged}
