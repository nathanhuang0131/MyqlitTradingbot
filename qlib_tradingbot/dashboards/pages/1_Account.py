...
def render(data_root: Path | str = "Data") -> None:
    ...
    st.caption(f"Decision blocks: {_fmt(m.get('decision_blocks'))}")
    ...
    health = view["position_health"]
    if health.empty:
        st.info("No cached position health data found (expected Data/position_health.csv).")
    else:
        st.dataframe(health.tail(100), use_container_width=True)

    alerts = view["risk_alerts"]
    if not alerts.empty:
