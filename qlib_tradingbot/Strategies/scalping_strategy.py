...
class ScalpingStrategy(StrategyBase):
    ...
    def execute(self, signals):
        ...
        fused_signals, decision_records = fuse_signals(
            [s for s in signals if isinstance(s, Signal)],
            llm_bias_state=refresh_bias_state(self.ctx.data_dir),
            run_id=str(self.ctx.run_id),
            correlation_id=str(self.ctx.correlation_id or self.ctx.run_id),
            strategy_id=str(self.strategy_id),
            market_regime=str((self.ctx.config or {}).get("market_regime", "neutral")),
            risk_state={
                "blocked": False,
                "min_qlib_confidence": float((self.ctx.config or {}).get("min_qlib_confidence", 0.05)),
            },
        )
        try:
            write_decision_trace(Path(self.ctx.data_dir), decision_records)
            upsert_position_journal(
                Path(self.ctx.data_dir),
                decision_records,
                expected_horizon=str((self.ctx.config or {}).get("expected_horizon", "1D")),
                entry_thesis=str((self.ctx.config or {}).get("entry_thesis", "scalping fused decision")),
                target=float((self.ctx.config or {}).get("default_target_price", 0.0)),
                stop=float((self.ctx.config or {}).get("default_stop_price", 0.0)),
            )
            review_position_health(Path(self.ctx.data_dir))
        except Exception as exc:
            self._monitor_log(
                stage="stage3_execution",
                status="fail",
                message="decision/position trace write failed",
                metrics={"orders_attempted": int(len(fused_signals)), "orders_submitted": 0, "orders_failed": int(len(fused_signals)), "dry_run": bool(effective_dry_run)},
                error=str(exc),
            )
            return []
        ...
        if not guard.ok:
            self._monitor_log(
                stage="stage3_execution",
                status="fail",
                message=f"guardrail blocked: {', '.join(guard.reasons)}",
                metrics={"orders_attempted": int(len(fused_signals)), "orders_submitted": 0, "orders_failed": int(len(fused_signals)), "dry_run": bool(effective_dry_run)},
            )
            return []
        ...
        if fused_signals and isinstance(fused_signals[0], Signal):
